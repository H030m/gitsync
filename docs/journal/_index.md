...
- **廷煥 — 前景通知點擊深連結到任務頁**：修「點通知掉到空殼 NotifyScreen」。後端早送 `{type:'task_ready',repoId,taskId}`、背景/冷啟動也早就會導任務頁——破口只在前景：本地通知 payload 只帶 taskId 缺 repoId、且 onTap 寫死 goNotify。前景 payload 改 `jsonEncode(m.data)`、onTap 改解析導頁；抽純函式 `taskRouteFromData`+`decodeNotificationPayload` 背景/前景共用。最小方案（只看 repoId+taskId，不做 daily 預留）。改動檔 analyze 0/0、`flutter test` **110/110**（+7 新測試）。trellis `06-16-deep-link-foreground-notification-taps-to-the-task`，分支 `feature/notify-deeplink-foreground`。詳見 [113062340_tinghuan.md](./113062340_tinghuan.md)。

## 2026-06-15

- **廷煥 — 任務看板列精簡（移除狀態圈圈、頭像移到左）**：團隊回饋手機看板列左側狀態圈圈多餘（長按整列就能換狀態）。改 `_SectionTaskRow`：拿掉狀態圈圈、頭像從右移左 → 一列＝頭像(左)+標題；移除無用的 `_markDone`/`isDone`；長按換狀態、點列導詳情保留。改動檔 analyze 0/0、`tasks_board_test` 11/11。⚠️ 拉最新 develop 後 `daily_*` 測試已紅（乾淨 develop 也紅＝嘉駿 chat/daily 重構合併帶進的既有問題，非本改動，請 owner 處理）。trellis `06-15-board-row-drop-status-circle-avatar-to-left`，分支 `feature/board-row-avatar-left`。詳見 [113062340_tinghuan.md](./113062340_tinghuan.md)。
