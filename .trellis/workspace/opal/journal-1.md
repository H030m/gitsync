...
## Session 19: 移除 Settings 頂端 Backend banner

**Date**: 2026-06-16
**Task**: 移除 Settings 頂端 Backend banner
**Branch**: `feature/remove-settings-backend-banner`

### Summary

拿掉 Settings 頁最上面的 _BackendBanner(Backend: LIVE/FAKE 資訊塊)+ 4 個專用 l10n 字串 + 未用 AppConfig import;General 卡成為第一項。AppConfig 後端切換邏輯不動。純 UI、119 tests 綠。另:放棄了 incremental-breakdown-force-submit task(未動程式)。
